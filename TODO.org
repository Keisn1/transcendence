...
- [ ] better error on user update

- [X] have a friends list in the dashboard view

- [X] double check input validation! (1. "Ensure that the website is secured. Check the database to verify that passwords are hashed. Check the server for server-side validation/sanitization on forms and any user input. If this isn't done, the evaluation ends now.")
  - https://www.esecurityplanet.com/endpoint/prevent-web-attacks-using-input-sanitization/
  - [X] make a list of every user input anywhere inside the app
  - [X] check every user input and sanitize and validate

- [X] sign up, show if a user is already present (if there is a conflict)

- [X] check for unhandled errors (no 500s if there is a good reason for failure)
  - [X] go through backend endpoints and check
  - [X] check with LLM
